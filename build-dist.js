...
// Archivos CSS
let dashmix_min = ""
let dashmix_min_length = 0

let xeco_min = ""
let xeco_min_length = 0

let fa_solid_900_woff2 = ""
let fa_solid_900_woff2_length = 0

let Simple_Line_Icons_woff2 = ""
let Simple_Line_Icons_woff2_length = 0

let fa_regular_400_woff2 = ""
let fa_regular_400_woff2_length = 0
...
console.log("INFO: ¡Converrtidor de archivos iniciado!")
...
.then((data) => {
    let { newData, length} = convertToHex(data)
    dashmix_app_min = newData
    dashmix_app_min_length = length
    return Promise.resolve()
}))
...
.then((data) => {
    let { newData, length} = convertToHex(data)
    fa_regular_400_woff2 = newData
    fa_regular_400_woff2_length = length
    return Promise.resolve()
}))
...
.then((data) => {
    let { newData, length} = convertToHex(data)
    fa_solid_900_woff2 = newData
    fa_solid_900_woff2_length = length
    return Promise.resolve()
}))
...
.then((data) => {
    let { newData, length} = convertToHex(data)
    Simple_Line_Icons_woff2 = newData
    Simple_Line_Icons_woff2_length = length
    return Promise.resolve()
}))
...
Promise.all(promises).finally(() => {
    ...
    #define xeco_css_length ${xeco_min_length}    
    const uint8_t xeco_css[] PROGMEM = {${xeco_min}};
    #define dashmix_css_length ${dashmix_min_length}    
    const uint8_t dashmix_css[] PROGMEM = {${dashmix_min}};
    #define fa_regular_woff2_length ${fa_regular_400_woff2_length}    
    const uint8_t fa_regular_woff2[] PROGMEM = {${fa_regular_400_woff2}};
    #define fa_solid_woff2_length ${fa_solid_900_woff2_length}    
    const uint8_t fa_solid_woff2[] PROGMEM = {${fa_solid_900_woff2}};
    #define Simple_Icons_woff2_length ${Simple_Line_Icons_woff2_length}    
